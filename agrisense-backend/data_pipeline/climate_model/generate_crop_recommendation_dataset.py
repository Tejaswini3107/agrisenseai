...
def generate_crop_recommendation_dataset():
    ...
    def get_crop_suitability(row):
        ...
        for crop, reqs in CROP_REQUIREMENTS.items():
            ...
            combined_score = temp_score * 0.3 + humidity_score * 0.25 + rainfall_score * 0.25 + vpd_score * 0.2
    ...
    df["disease_risk_numeric"] = df.apply(lambda row: calculate_disease_risk(row, row["best_crop"]), axis=1)
    df["irrigation_need"] = df.apply(lambda row: calculate_irrigation_need(row, row["best_crop"]), axis=1)
    df["plant_stress"] = df.apply(lambda row: calculate_plant_stress(row, row["best_crop"]), axis=1)
