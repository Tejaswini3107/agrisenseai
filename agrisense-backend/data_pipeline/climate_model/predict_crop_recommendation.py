...
def get_crop_recommendation(weather_features):
    ...
    if disease_risk == "High":
        print("    Recommendation: Monitor crop closely, consider preventive spraying")
    elif disease_risk == "Medium":
        print("    Recommendation: Maintain good farm hygiene, watch for symptoms")
    else:
    ...
    if plant_stress == "High":
        ...
        print("     Recommendation: Monitor water availability, may need irrigation soon")
    else:
    ...
    if reqs["temp_min"] <= weather_features["temperature"] <= reqs["temp_max"]:
        factors.append(
            f"    Temperature {weather_features['temperature']:.1f}°C (Optimal: {reqs['temp_min']}-{reqs['temp_max']}°C)"
        )
    else:
        factors.append(
            f"    Temperature {weather_features['temperature']:.1f}°C (Optimal: {reqs['temp_min']}-{reqs['temp_max']}°C)"
    ...
    if reqs["humidity_min"] <= weather_features["humidity"] <= reqs["humidity_max"]:
        factors.append(
            f"    Humidity {weather_features['humidity']:.1f}% (Optimal: {reqs['humidity_min']}-{reqs['humidity_max']}%)"
        )
    else:
        factors.append(
            f"    Humidity {weather_features['humidity']:.1f}% (Optimal: {reqs['humidity_min']}-{reqs['humidity_max']}%)"
    ...
    if reqs["rainfall_min"] <= weather_features["rainfall_mm"] <= reqs["rainfall_max"]:
        factors.append(
            f"    Rainfall {weather_features['rainfall_mm']:.2f}mm (Optimal: {reqs['rainfall_min']}-{reqs['rainfall_max']}mm)"
        )
    else:
        factors.append(
            f"   ✗ Rainfall {weather_features['rainfall_mm']:.2f}mm (Optimal: {reqs['rainfall_min']}-{reqs['rainfall_max']}mm)"
    ...
    if reqs["vpd_min"] <= weather_features["vapor_pressure_deficit"] <= reqs["vpd_max"]:
        factors.append(
            f"    VPD {weather_features['vapor_pressure_deficit']:.2f} (Optimal: {reqs['vpd_min']}-{reqs['vpd_max']})"
        )
    else:
        factors.append(
            f"    VPD {weather_features['vapor_pressure_deficit']:.2f} (Optimal: {reqs['vpd_min']}-{reqs['vpd_max']})"
