...
def train_lstm_weather_forecast():
    ...
    model = Sequential(
        [
            ...
            layers.LSTM(
                64, activation="relu", return_sequences=True, input_shape=(sequence_length, num_features), name="lstm_1"
            ),
            layers.Dropout(0.2),
    ...
    for i, feature in enumerate(weather_features):
        ...
        y_pred_train_original[:, :, i] = scaler.inverse_transform(y_pred_train_reshaped).reshape(
            y_pred_train[:, :, i].shape
        )
        y_test_original[:, :, i] = scaler.inverse_transform(y_test_reshaped).reshape(y_test[:, :, i].shape)
        y_pred_test_original[:, :, i] = scaler.inverse_transform(y_pred_test_reshaped).reshape(
            y_pred_test[:, :, i].shape
        )
    ...
    for i, feature in enumerate(weather_features):
        ...
        feature_rmse = np.sqrt(
            mean_squared_error(y_test_original[:, :, i].reshape(-1), y_pred_test_original[:, :, i].reshape(-1))
        )
