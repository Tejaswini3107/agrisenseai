...
print("\nHeat Index:")
hi_min = df_engineered['heat_index'].min()
hi_max = df_engineered['heat_index'].max()
print(f"  Range: {hi_min:.2f} to {hi_max:.2f}")
heat_index_valid = (
    (df_engineered['heat_index'] >= 10).all()
    and (df_engineered['heat_index'] <= 80).all()
)
print(f"  All values in [10, 80]? {heat_index_valid}")
print(f"  Values: {df_engineered['heat_index'].tolist()}")

print("\nDew Point:")
dp_min = df_engineered['dew_point'].min()
dp_max = df_engineered['dew_point'].max()
print(f"  Range: {dp_min:.2f} to {dp_max:.2f}°C")
print(f"  Values: {df_engineered['dew_point'].tolist()}")

print("\nVapor Pressure Deficit:")
vpd_min = df_engineered['vapor_pressure_deficit'].min()
vpd_max = df_engineered['vapor_pressure_deficit'].max()
print(f"  Range: {vpd_min:.4f} to {vpd_max:.4f}")
print("  All rounded to 4 decimals? ", end="")
...
print(df_edge_eng[edge_cols])

print("\n6. Feature Statistics")
print("-" * 70)
print("\nDescriptive Statistics:")
stat_cols = ["heat_index", "dew_point", "vapor_pressure_deficit"]
print(df_engineered[stat_cols].describe())
...
all_cols_present = all(col in df_engineered.columns for col in expected_columns)
no_data_loss = len(df_engineered) == len(df_original)
heat_clipped = (
    (df_engineered['heat_index'] >= 10).all()
    and (df_engineered['heat_index'] <= 80).all()
)
rainfall_type_ok = df_engineered['rainfall_category'].dtype == 'object'
flags_are_int = (
    df_engineered['is_high_humidity'].dtype in ['int64', 'int32']
